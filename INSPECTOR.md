...
To inspect an MCP server implementation, there's no need to clone this repo. Instead, use `npx`. For example, if your server is built at `dist/mcp/MCP.js`:
...
```bash
# Pass arguments only
npx @modelcontextprotocol/inspector node dist/mcp/MCP.js arg1 arg2

# Pass environment variables only
npx @modelcontextprotocol/inspector -e key=value -e key2=$VALUE2 node dist/mcp/MCP.js

# Pass both environment variables and arguments
npx @modelcontextprotocol/inspector -e key=value -e key2=$VALUE2 node dist/mcp/MCP.js arg1 arg2

# Use -- to separate inspector flags from server arguments
npx @modelcontextprotocol/inspector -e key=$VALUE -- node dist/mcp/MCP.js -e server-flag
```

The inspector runs both an MCP Inspector (MCPI) client UI (default port 6274) and an MCP Proxy (MCPP) server (default port 6277). Open the MCPI client UI in your browser to use the inspector. (These ports are derived from the T9 dialpad mapping of MCPI and MCPP respectively, as a mnemonic). You can customize the ports if needed:

```bash
CLIENT_PORT=8080 SERVER_PORT=9000 npx @modelcontextprotocol/inspector node dist/mcp/MCP.js
```
...
```bash
DANGEROUSLY_OMIT_AUTH=true yarn start
```
...
```bash
MCP_PROXY_AUTH_TOKEN=$(openssl rand -hex 32) yarn start
```
...
```bash
ALLOWED_ORIGINS=http://localhost:6274,http://localhost:8000 yarn start
```
...
```json
{
  "mcpServers": {
    ...
    "my-server": {
      "command": "node",
      "args": ["dist/mcp/MCP.js", "arg1", "arg2"],
      "env": {
        "key": "value",
        "key2": "value2"
      }
...
```bash
npx @modelcontextprotocol/inspector --cli node dist/mcp/MCP.js
```
...
npx @modelcontextprotocol/inspector --cli node dist/mcp/MCP.js --method tools/list

# Call a specific tool
npx @modelcontextprotocol/inspector --cli node dist/mcp/MCP.js --method tools/call --tool-name mytool --tool-arg key=value --tool-arg another=value2

# Call a tool with JSON arguments
npx @modelcontextprotocol/inspector --cli node dist/mcp/MCP.js --method tools/call --tool-name mytool --tool-arg 'options={"format": "json", "max_tokens": 100}'

# List available resources
npx @modelcontextprotocol/inspector --cli node dist/mcp/MCP.js --method resources/list

# List available prompts
npx @modelcontextprotocol/inspector --cli node dist/mcp/MCP.js --method prompts/list
